...
public class OperatorMouseListener extends BlockMouseListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = -4666350146086219846L;

	public OperatorMouseListener(AlgorithmPanel panel) {
		super(panel);
	}
			...
			if (e.getClickCount() == 2) {
				...
				if (e.getClickCount() == 1) {
					if (!model.contains(e.getX(), e.getY())) {
						model.addComponent(new Operator(e.getX(), e.getY()));
					} else {
