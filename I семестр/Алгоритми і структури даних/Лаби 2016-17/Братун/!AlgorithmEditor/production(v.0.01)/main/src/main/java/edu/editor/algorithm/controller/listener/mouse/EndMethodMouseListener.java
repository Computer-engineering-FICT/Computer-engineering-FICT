...
public class EndMethodMouseListener extends BlockMouseListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = -478937959068575184L;

	public EndMethodMouseListener(AlgorithmPanel panel) {
		super(panel);
	}
			...
			if (e.getClickCount() == 2) {
				...
				if (e.getClickCount() == 1) {
					if (!model.contains(e.getX(), e.getY())) {
						if (!isEndBlock()) {
							model.addComponent(new EndMethod(e.getX(), e.getY()));
						} else {
