...
public class ConditionMouseListener extends BlockMouseListener {
	/**
	 * 
	 */
	private static final long serialVersionUID = -8101973863341952226L;

	public ConditionMouseListener(AlgorithmPanel panel) {
		super(panel);
	}
			...
			if (e.getClickCount() == 2) {
				...
				if (e.getClickCount() == 1) {
					if (!model.contains(e.getX(), e.getY())) {
						model.addComponent(new Condition(e.getX(), e.getY()));
					} else {
