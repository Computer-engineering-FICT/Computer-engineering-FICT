    ...
    package Package1 is
    	N: integer := 4000;   --size of structures
  ...
  package body Package1 is
   ...
   procedure Sort (V : in out Vec) is
       Temp  : Integer;
begin
       temp := V(1);
       -- сортування бульбашкою ---
       for Count1 in V'Range loop
          for Count2 in V'Range loop
             if (V(Count1) > V(Count2)) then
                begin
                   Temp      := V(Count1);
                   V(Count1) := V(Count2);
                   V(Count2) := Temp;
