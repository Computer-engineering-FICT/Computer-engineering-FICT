...
//        ShowMessage(intTostr(i)+'  '+intToStr(VerList[i].NextMain))
      end;
      if VerList[i].NextAlt<>-1 then
      begin
        TableSv[VerList[i].num,VerList[i].NextAlt]:=true;
//        ShowMessage(intTostr(i)+'  '+intToStr(VerList[i].NextAlt))
  ...
  until not change;
   ...
   for i:=0 to MaxVerNum do
     if i<>EndVer then
       ...
       begin
         flag:=false;
         ShowMessage('# '+IntToStr(i)+' has no connection with the End ');
//         ShowMessage('Вершина №'+IntToStr(i)+' не имеет связи с конечной вершиой ');
       end;
  TestConnect:=Flag;
end;{TestConnect}
  ...
  begin
    ...
    case VerList[FindVer(i)].Vid of
      b:if ConCount<1 then
        begin
          ShowMessage('#0 has no connection !');
//          ShowMessage('Начальная вршина не имеет связи !');
          CorrectConnect:=false
        end;
      o:if ConCount<1 then
        begin
          ShowMessage('#'+inttostr(i)+ ' has no connections !!');
...
//          ShowMessage('Вершина №'+inttostr(i)+ 'не имеет связи !');
      ...
      l:if ConCount<2 then
        ...
        begin
          ShowMessage('Both branches of # '+inttostr(i)+' follow to # '+IntTostr(VerList[i].NextMain));
          CorrectConnect:=false
...
//ShowMessage('ComplPathCount '+intToStr(ComplPathCount ));
   ...
   begin
     ...
     begin
       AllInpath:=false;
       ShowMessage('# '+IntToStr(VerList[i].num)+' not belongs to any path or cycle ')

//       ShowMessage('Вершина № '+IntToStr(i)+' не входит ни в один путь или цикл ')
     end
   end ;
end;{AllInpath}
...
begin
  ...
  begin
    ...
    if not IsOp then
    begin
      ShowMessage('Cycle #'+intToStr(i)+' has no operators');
...
//      ShowMessage('Цикл №'+intToStr(i)+' не содержит операторных вершин');
    ...
    if not IsLog then
    begin
      ShowMessage('Cycle #'+intToStr(i)+' has no conditions');
