...
procedure TForm3.Button1Click(Sender: TObject);{insert}
  ...
  begin
    ...
    repeat
      ...
      if (RadioGroup1.ItemIndex=2)and(First[1]<>'X')then
        TestSignal:=false;
      if (RadioGroup1.ItemIndex=1)and(First[1]<>'Y')then
        TestSignal:=false;
      until Last=''
...
begin
     if (Trim(Edit1.Text)='')and(RadioGroup1.ItemIndex<>0) then
        MessageDlg('Input the logical signal of the point',mtWarning,[mbok],0)
       //ShowMessage('Вершина должна содержать сигнал !')
     else
     if not TestSignal then
       messageDlg('Signal is not correct',mtInformation,[mbOK],0)
     //  ShowMessage('Сигнал не верно задан')
     else
     begin
     flag:=false;
