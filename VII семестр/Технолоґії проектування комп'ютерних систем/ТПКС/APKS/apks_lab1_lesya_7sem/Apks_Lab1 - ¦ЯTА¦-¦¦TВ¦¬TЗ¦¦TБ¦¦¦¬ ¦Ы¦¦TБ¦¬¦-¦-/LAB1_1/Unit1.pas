...
begin
  ...
  begin
    ...
    Form1.Image1.Canvas.TextOut(Coord[3].x, Coord[3].Y - Form1.Image1.Canvas.TextHeight('нет') - 3, 'нет');
    Form1.Image1.Canvas.TextOut(Coord[4].x + 3, Coord[4].Y, 'да');
...
begin
  ...
  begin
    ...
    if VerList[i].Vid=l then
      ...
      begin
        empty:=false;
        Form4.cbPrev.Items.Add(IntToStr(VerList[i].Num)+'/ альтернативная ветвь');
      end;
  end;
  if empty then
    showError('Невозможно добавить связь в граф !')
  else
  begin
    for i:=1 to MaxVerNum do
      if VerList[i].exist then
