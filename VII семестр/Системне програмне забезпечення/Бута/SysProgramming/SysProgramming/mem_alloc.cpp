...
* checkSum - контрольна сума даних в блоці.
* size - розмір блоку.
...
/**
* Пошук підходящого блоку для виділення пам'яті з найменшим розміром.
* size - розмір пам'яти, що має виділятися.
* returns Вказівник на знайдений блок або NULL, якщо блоку не знайдено.
*/
Header *minSizeSAlgorithm(size_t size) {
    ...
    // Знаходження потрібного блоку мінімального розміру
    for ( int i = 1, last = blocksQuantity - 2; i <= last; i++ ) {
    ...
    // Якщо потрібного блоку не знайдено
    if ( minBlockIndex == -1 ) {
        return NULL;
    }

    // Якщо потрібний блок знайдений, переходимо на заголовок цього блоку
