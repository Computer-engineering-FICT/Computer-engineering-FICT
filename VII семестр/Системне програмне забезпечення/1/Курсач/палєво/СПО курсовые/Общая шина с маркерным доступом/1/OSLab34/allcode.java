...
public class Controller {
	...
	public void unlink(int id, String name) {
        ...
        LinkNameRecord link = new LinkNameRecord(maxVer, currId, 
                (byte) (name.length() + 1), LinkNameRecord.DEL_CHAR + name);
        maxVer++;
        ArrayList<Record> arr = buffer.writeRecord(link);
        table.applyRecord(arr.get(0));
	}
	 ...
	 * @return 	if not enough space on flash - FALSE
	 */
	public boolean write (int id, int shift, int size, byte[] dat) {
		if (buffer.getFreeBlockCount() <= Params.REQ_FREE_BLOCKS_COUNT + 1) {
			buffer.clean();
		}
		
		//===============Free space control================
		int occurExtraSize = 0;
		
		int extraBlocks = 0;
		
		int recSize = (new WriteRecord(0, 0, 0, 0, null)).getRecordSize();
		
		if (buffer.getBufferFreeSpace() > recSize + 1) {
			occurExtraSize = dat.length - buffer.getBufferFreeSpace() + recSize; 
		}  else {
			occurExtraSize = dat.length;
		}
		
		if (occurExtraSize > 0) {
			extraBlocks = occurExtraSize / (Params.BLOCK_SIZE - recSize);
			if (occurExtraSize % (Params.BLOCK_SIZE - recSize) > 0) extraBlocks++;
		}
		
		//=======================WRITE======================
		if (extraBlocks + 1 <= buffer.getFreeBlockCount() - 
				Params.REQ_FREE_BLOCKS_COUNT) 
		{		
			int currId = table.getFileBySysID(id).getId();
	...
	public byte[] read(int id, int shift, int size) {
        ...
        for (PartDescriptor p : parts) {
        	int startPos = p.getFlashShift() + 
				(new WriteRecord(0, 0, 0, 0, null)).getRecordSize();
        	for (int i = startPos; i < startPos + p.getSize(); i++) 
        	{
        		rez[pos] = buffer.readByte(i);
        		pos++;
        	}
        }
	...
	public int getFreeBlockCount() {
		return buffer.getFreeBlockCount();
	}
	...
	public boolean containsName(String name) {
		return table.containsName(name);
	}
	...
	public int getFreeBufSize() {
		return buffer.getBufferFreeSpace();
	}
}
...
public class Buffer {
	...
	public int getFreeBlockCount() {
		...
		for (int i = 0; i < Params.FLASH_SIZE; i++) {
			if (getBlockState(i) == FREE_BLOCK) rez++;
		}
	...
	/**
	 * Flush buffer to flash. If buffer isn't full, then in record queue is 
	 * recorded.
	 */
	public void flushBuffer() {	
		try {
		...
		for (int i = 0; i < Params.FLASH_SIZE; i++) {	
			...
			if (flag) {
				...
				for (Record rec : recQueue) {
					...
					if (rec.getCode() == Record.WRITE) {
						size += ((WriteRecord) rec).getAveralSize();
					} else {
						size += rec.getRecordSize();
					}
				}
				
				//if block is clean but not full  - rewrite it
				if (!dirty && size < Params.BLOCK_SIZE) {
					rezQueue = recQueue;
					dirty = true;
				}
	...
	/**
	 * Examine record for actual information.
	 * @param rec		testing record
	 * @param rezQueue	result queue
	 * @return			if dirty - TRUE, if clean - FALSE
	 */
	private boolean getActualRec(Record rec, ArrayList<Record> rezQueue) {
		...
		if (!desc.isDeleted()) {
			if (rec.getCode() == Record.LINK_NAME) {
				...
				//if name is signal for delete name (starts with '&') or name 
				//is already deleted - we don't save this record				
	...
	public static String byteArrToStr(byte[] dat) {
	    StringBuffer buf = new StringBuffer();
		
	    for (int i = 0; i < dat.length; i++) {
	    	buf.append((char) dat[i] + " ");
	    }
		
		return buf.toString();
	}
	
	public byte readByte(int shift) {
		byte rez = 0;
		
		if (shift < Params.BLOCK_SIZE * currentBlockNum || 
				shift > Params.BLOCK_SIZE * (currentBlockNum + 1)) {
			try {
				file.seek(shift);
				rez = file.readByte();
			} catch (IOException e) {
			...
			rez = buffer.toByteArray()[shift - Params.BLOCK_SIZE * currentBlockNum];
		}
	...
	public int getBufferFreeSpace() {
		return Params.BLOCK_SIZE - stream.size();
	}
}
...
/**
 *	LinkName descriptor. If field starts with '&' (DEL_CHAR) character - it's unlink record
 * 	Type		Code	Fields
 * 	Link name	0x1D	code ver fileId	nameLen fileName
 * @author Pustovit Michael, pustovitm@gmail.com
 */
public class LinkNameRecord extends Record { 
	public static char DEL_CHAR = '&';
...
public class Params {
    ...
    public static int FLASH_SIZE    = 5;
