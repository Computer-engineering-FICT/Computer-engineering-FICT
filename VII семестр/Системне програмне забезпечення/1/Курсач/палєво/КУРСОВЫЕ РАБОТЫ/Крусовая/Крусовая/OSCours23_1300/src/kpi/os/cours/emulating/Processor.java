...
public class Processor {
	...
	private HashSet<Task> buffer;
	
	public Processor(int num, ArrayList<Task> tasks, ProcessorPool procPool) {
		this.num = num;
		...
		this.procPool = procPool;
		buffer = new HashSet<Task>();
	}
			...
			if (remainProcTime == 0) {
				for (Edge e : currentTask.getOutEdges()) {
					if (!allTasks.contains(e.getEnd())) {
						...
						Collections.sort(sendQueue, new Comparator<Pair>() {

							@Override
							public int compare(Pair o1, Pair o2) {								
								return o2.edge.getEnd().getStatPath() - 
									o1.edge.getEnd().getStatPath();
							}
						});
					} else {
	...
	public void manageTrans() {
		...
		if (remainTransTime > 0) {
			...
			if (remainTransTime == 0) {				
				Processor receiver = procPool.getTaskProcessor(
						currentTrans.edge.getEnd());
				receiver.forceReceive(currentTrans);
				currentTrans = null;
			}
		}
	}
	...
	private void tryScheduleSend() {
		if (sendQueue.size() > 0) {
			sendQueue.get(0).setStartTime(curTime);
	...
	public void forceReceive(Pair transition) {
		...
		history.add(new Interval(
				transition.getStartTime(), 
				transition.getStartTime() + transition.edge.getWeight(), 
				IntervalType.RECEIVING, 
				Integer.toString(transition.task.getNum())));
		
		if (procPool.getTaskProcessor(transition.task).getNum() > num) {
			receivedData.add(transition.task);
		} else {
			buffer.add(transition.task);
		}
