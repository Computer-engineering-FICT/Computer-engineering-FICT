...
public class TaskGraph {
			...
			ClusterPool pool) 
	{
		...
		for (Edge t : curTask.getOutEdges()) {
			...
			if (currentCl == null && childCl == null) {
				newPath =	curTask.getWeight() +
							t.getWeight() + 
							t.getEnd().getPath();
			} else
			
			//If current task in cluster, child - isn't in cluster
			if (currentCl != null && childCl == null) {
				newPath	= 	currentCl.getAboveWeight(curTask) + 
							t.getWeight() + 
							t.getEnd().getPath();
			} else
			...
			if (currentCl == null && childCl != null) {
				newPath = 	t.getEnd().getPath() + 
							t.getWeight() + 
							curTask.getWeight();
			} else
				
			//If tasks are in one cluster - all edges between them are zeroed  
			if (currentCl.equals(childCl)) {
				newPath = currentCl.getAboveWeight(t.getEnd());				
			} 
			//============================================
		...
		for (Edge t : curTask.getInEdges()) {
			int temp = recursPass(t.getEnd(), visited, pool); 
	...
	public int getCriticalPass(ClusterPool pool) {
		...
		return recursPass(root, visited, pool);
	}
	//==============================================
