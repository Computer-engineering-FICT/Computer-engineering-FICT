...
begin
  ...
  begin
    ...
    Form1.Image1.Canvas.TextOut(Coord[3].x, Coord[3].Y - Form1.Image1.Canvas.TextHeight(ALT_NO) - 3, ALT_NO);
    Form1.Image1.Canvas.TextOut(Coord[4].x + 3, Coord[4].Y, ALT_YES);
    ...
    if VerList[i].exist then
      ...
      begin
        ...
        if (VerList[i].Vid=l) then
          Form3.ComboBox1.Items.Add(IntToStr(VerList[i].Num) + ALT_SEPARATOR + ' ' + ALT_YES) else
      ...
      if VerList[i].Vid=l then
        ...
        begin
          empty:=false;
          Form3.ComboBox1.Items.Add(IntToStr(VerList[i].Num) + ALT_SEPARATOR + ' ' + ALT_NO);
        end;
    end;
  if empty then
   showError('Невозможно добавить вершину в граф')
  else
  begin
...
begin
  ...
  begin
    ...
    Form4.lbPrev.Caption:='Удаляемая вершина';
    Form4.lbPrev.Visible := True;
    Form4.Caption:='Удаление';
    Form4.ShowModal;
  end
  else ShowError('Нечего удалять');
  Form4.Free;
end;
...
begin
  ...
  begin
    ...
    begin
      empty:=false;
      if VerList[i].Vid=l then
        Form4.cbPrev.Items.Add(IntToStr(VerList[i].Num) + ALT_SEPARATOR + ' ' + ALT_YES) else
        Form4.cbPrev.Items.Add(IntToStr(VerList[i].Num));
    end;
    if VerList[i].Vid=l then
      if VerList[i].NextAlt=-1 then
      begin
        empty:=false;
        Form4.cbPrev.Items.Add(IntToStr(VerList[i].Num)+ALT_SEPARATOR + ' ' + ALT_NO);
      end;
  end;
  // No out comes
  if empty then
  begin
    showError('Нет вершин с неподключенными выходами');
    Exit;
  end;
  // No enters
    for i:=1 to MaxVerNum do
  ...
  if Form4.cbNext.Items.Count = 0 then
  begin
    showError('Нет вершин с неподключенными входами');
    Exit;
  end;
  // Good
  begin
    ...
    Form4.lbPrev.Visible:=true;
    Form4.lbNext.Caption:='Следующая вершина';
    Form4.Caption:='Связывание';
    Form4.ShowModal;
    Form4.Free;
  end;
end;
...
begin
  ...
  if empty then
    showError('Граф не содержит связей')
  else
  begin
    for i:=1 to MaxVerNum do
    if VerList[i].exist then
...
begin
  ...
  begin
    ...
    AllRight:=false;
if AllRight then
  ShowInfo('Граф проверен, ошибок не найдено');
