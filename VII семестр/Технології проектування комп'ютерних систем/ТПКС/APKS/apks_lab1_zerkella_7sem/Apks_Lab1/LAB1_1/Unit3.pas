...
function IsNoDelim(delimiter:char;S:string):boolean;
...
//          EndVerNum:=CurrentVer
     ...
     Vp:=NumFromStr(ComboBox1.Items[ComboBox1.ItemIndex],ALT_SEPARATOR);
     SetLength(Prev,1);
     Prev[0]:=Vp;
   end;
   with VerList[Vp] do
   begin
     if (Vid=o) or (Vid=b) then
       NextMain:=CurrentVer;
     if Vid=l then
       if IsNoDelim(ALT_SEPARATOR,ComboBox1.Items[ComboBox1.ItemIndex])  then
         NextAlt:=CurrentVer
      else
         Nextmain:=CurrentVer;
     case Vid of
