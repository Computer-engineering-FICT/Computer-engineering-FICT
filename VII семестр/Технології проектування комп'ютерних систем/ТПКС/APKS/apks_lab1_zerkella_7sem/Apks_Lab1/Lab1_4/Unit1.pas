...
{----------------------------------}
procedure Connect2(TrigX,TrigY,Vhod,Nom,X,Y,VhodCnt:integer; Signal: string);
  var x1,y1,x2,y2,x3,y3,x4,y4:integer;
  OldColor: TColor;
begin
  x1:=TrigX;
  ...
  if X=-1 then
  begin
    OldColor := Form1.Image1.Canvas.Font.Color;
    Form1.Image1.Canvas.Font.Color := clNavy;
    Form1.Image1.Canvas.TextOut(x2-10,y2-7,Signal);
    Form1.Image1.Canvas.Font.Color := OldColor;
  end;
end;
...
{----------------------------------}
procedure PutTrigger(Text:string;InText: string; Nom:integer);
begin
  ...
  Form1.Image1.Canvas.TextOut(Left+wideth div 2,Top+Height div 2,InText);
...
Form1.Image1.Canvas.Pen.Width := 2;
  ...
  if (TrigName='T')or (TrigName='D') then
    for i:=0 to TrigCnt-1 do
      PutTrigger(TrigName,IntToStr(I), i)
  else
    for i:=0 to TrigCnt-1 do
      PutTrigger(TrigName,IntToStr(I),i*2);
end;{Draw}
{---------------------------------------}
procedure Init;
  var i:integer;
...
Form1.Image1.Canvas.Pen.Width := 1;
  ...
  for i:=0 to Trigcnt-1 do
      if  (TrigName='T')or (TrigName='D') then
        Connect2(Funcs[i].xt,Funcs[i].yt,0,i,Funcs[i].x,Funcs[i].y,length(Funcs[i].TermList), Funcs[I].Name)
      else
      begin
        Connect2 (Funcs[2*i].xt,Funcs[2*i].yt,1,2*i,Funcs[i*2].x,Funcs[i*2].y,length(Funcs[2*i].TermList), Funcs[2*I].Name );
        Connect2 (Funcs[2*i].xt,Funcs[2*i].yt,2,2*i+1,Funcs[2*i+1].x,Funcs[2*i+1].y,length(Funcs[2*i+1].TermList), Funcs[2*I + 1].Name  )
      end;
