                        ...
                        if prov2 then begin
                          ...
                          if (num_cond>=1) and (num_link>=1) then begin
                            ...
                            for l:=cur_cycle_num downto 1 do
                              s := s + inttostr(cur_cycle[l])+'->';
                            MainGraphForm.mResult.lines.Add(s);
                          end else
                          if (num_cond>=1) and (num_link<1) then begin
                              s:='Цикл ';
                              for l:=cur_cycle_num downto 1 do
                              s := s + inttostr(cur_cycle[l])+'->';
                            MainGraphForm.mResult.lines.Add(s);
                            raise EGraphCheckError.Create(CycleWithNoOper,-1,cur_cycle[cur_cycle_num])
                          end else
                          if (num_cond<1) and (num_link>=1) then begin
                            s:='Цикл ';
                            for l:=cur_cycle_num downto 1 do
                              s:=s+inttostr(cur_cycle[l])+'.';
                            MainGraphForm.mResult.lines.Add(s);
                            raise EGraphCheckError.Create(CycleWithNoCond,-1,cur_cycle[cur_cycle_num])
                          end;
                        end;
                       end;
                      end;
...
begin
  ...
  for i:=1 to VershCol do
    for j:=1 to VershCol do begin
      Perehodi[j,i]:=MainGraphForm.StringGrid1.Cells[j,i];
    end;
  for i:=1 to VershCol do
    for j:=1 to 3 do
      Signali[j,i]:=MainGraphForm.StringGrid2.Cells[j,i];
end;
...
var i,j:integer;
begin
  MainGraphForm.StringGrid1.RowCount:=VershCol+1;
  MainGraphForm.StringGrid1.ColCount:=VershCol+1;
  MainGraphForm.StringGrid2.RowCount:=VershCol+1;
  for i:=1 to VershCol do begin
    MainGraphForm.StringGrid1.Cells[0,i]:=inttostr(i);
    MainGraphForm.StringGrid1.Cells[i,0]:=inttostr(i);
    MainGraphForm.StringGrid2.Cells[0,i]:=inttostr(i);
  end;
  for i:=1 to VershCol do
    for j:=1 to VershCol do
      MainGraphForm.StringGrid1.Cells[j,i]:=Perehodi[j,i];
  for i:=1 to VershCol do
    for j:=1 to 3 do
      MainGraphForm.StringGrid2.Cells[j,i]:=Signali[j,i];
end;
...
begin
  ...
  reset(f);
  readln(f,VershCol);
  MainGraphForm.edit1.text:=inttostr(vershcol);
  for i:=1 to VershCol do
    for j:=1 to VershCol do
      MainGraphForm.StringGrid1.Cells[j,i]:='';
  for i:=1 to VershCol do
    for j:=0 to 3 do
      MainGraphForm.StringGrid2.Cells[j,i]:='';
  for i:=1 to VershCol do begin
    Block_Coord[i,1]:=0;
    Block_Coord[i,2]:=0;
  end;
...
var cur_way_num,i,g,m,num_way,cur_mas,cur_mas1:integer;
...
begin
  ...
  if MainGraphForm.RadioGroup3.ItemIndex=0 then begin
    //автомат Мили
    i:=0;
    repeat
      inc(i);
    ...
    for i:=1 to SostNum do
      for j:=1 to SostNum do begin
        ...
        for o:=1 to way[0,0] do begin
          ...
          avtomat_mili[i].perehodi[strtoint(avtomat_mili[i].perehodi[0,0]),-1]:='0';
    ...
    MainGraphForm.stringgrid3.ColCount:=SostCol+1;
    MainGraphForm.stringgrid3.RowCount:=TrigCol+2;
    MainGraphForm.stringGrid3.Cells[0,0]:='Вершина';
    MainGraphForm.stringGrid3.Cells[0,1]:='Состояние';
    for i:=1 to TrigCol do
      MainGraphForm.stringGrid3.Cells[0,i+1]:='  Q'+inttostr(i);
    for i:=1 to SostCol do begin
      MainGraphForm.StringGrid3.Cells[i,0]:=inttostr(razm_sost[i,0]);
      MainGraphForm.StringGrid3.Cells[i,1]:='a'+inttostr(razm_sost[i,1]);
      for j:=2 to TrigCol+1 do begin
        if length(dectobin(razm_sost[i,1]-1))>TrigCol+1-j then begin
          ...
          if sttt[j-1-TrigCol+length(sttt)]='0' then
            MainGraphForm.StringGrid3.Cells[i,j]:='0'
          else MainGraphForm.StringGrid3.Cells[i,j]:='1';
        end else MainGraphForm.StringGrid3.Cells[i,j]:='0';
      end;
    end;

  //структурная таблица автомата в СГ

    MainGraphForm.StringGrid4.colcount:=TrigCol*2+2;
    MainGraphForm.StringGrid4.cells[0,0]:='ИС';
    for i:=1 to TrigCol do
      MainGraphForm.StringGrid4.cells[i,0]:='Q'+inttostr(i);
    MainGraphForm.StringGrid4.cells[TrigCol+1,0]:='СП';
    for i:=1 to TrigCol do
      MainGraphForm.StringGrid4.cells[i+TrigCol+1,0]:='Q'+inttostr(i)+'+';
    ...
    for o:=1 to VershCol do
      for g:=1 to 3 do
        ...
        MainGraphForm.stringgrid4.ColCount:=MainGraphForm.stringgrid4.ColCount+mas_x[0]+mas_y[0];
        for g:=1 to mas_x[0] do
          MainGraphForm.stringgrid4.cells[g+TrigCol*2+1,0]:='x'+inttostr(mas_x[g]);
        for g:=1 to mas_y[0] do
          MainGraphForm.stringgrid4.cells[g+TrigCol*2+1+mas_x[0],0]:='y'+inttostr(mas_y[g]);
        if MainGraphForm.radiogroup4.ItemIndex=0 then begin
          MainGraphForm.stringgrid4.ColCount:=MainGraphForm.stringgrid4.ColCount+TrigCol;
          for o:=MainGraphForm.stringgrid4.ColCount-TrigCol to MainGraphForm.stringgrid4.ColCount-1 do
            MainGraphForm.stringgrid4.Cells[o,0]:=' D'+inttostr(o-(MainGraphForm.stringgrid4.ColCount-TrigCol)+1);
        end else begin
          MainGraphForm.stringgrid4.ColCount:=MainGraphForm.stringgrid4.ColCount+TrigCol*2;
          for o:=MainGraphForm.stringgrid4.ColCount-2*TrigCol to MainGraphForm.stringgrid4.ColCount-1 do
            if ((o-(MainGraphForm.stringgrid4.ColCount-2*TrigCol)+1) mod 2)=1 then
              MainGraphForm.stringgrid4.Cells[o,0]:=' R'+inttostr((o-(MainGraphForm.stringgrid4.ColCount-2*TrigCol)) div 2+1)
            else MainGraphForm.stringgrid4.Cells[o,0]:=' S'+inttostr((o-(MainGraphForm.stringgrid4.ColCount-2*TrigCol)) div 2+1);
        end;
    ...
    for i:=1 to SostNum do
      for j:=1 to strtoint(avtomat_mili[i].perehodi[0,0]) do begin
          MainGraphForm.stringGrid4.RowCount:=cur_mas+1;
          MainGraphForm.stringGrid4.cells[0,cur_mas]:=' a'+inttostr(i);
          for g:=2 to TrigCol+1 do begin
            if length(dectobin(i-1))>TrigCol+1-g then begin
              sttt:=dectobin(i-1);
              if sttt[g-1-TrigCol+length(sttt)]='0' then
                MainGraphForm.StringGrid4.Cells[g-1,cur_mas]:='0'
              else MainGraphForm.StringGrid4.Cells[g-1,cur_mas]:='1';
            end else MainGraphForm.StringGrid4.Cells[g-1,cur_mas]:='0';
          end;
          MainGraphForm.stringGrid4.cells[TrigCol+1,cur_mas]:=' a'+avtomat_mili[i].perehodi[j,0];
          for g:=2 to TrigCol+1 do begin
            if length(dectobin(strtoint(avtomat_mili[i].perehodi[j,0])-1))>TrigCol+1-g then begin
              ...
              if sttt[g-1-TrigCol+length(sttt)]='0' then
                MainGraphForm.StringGrid4.Cells[g+TrigCol,cur_mas]:='0'
              else MainGraphForm.StringGrid4.Cells[g+TrigCol,cur_mas]:='1';
            end else MainGraphForm.StringGrid4.Cells[g+TrigCol,cur_mas]:='0';
          end;
          if avtomat_mili[i].perehodi[j,-1]='0' then
            for o:=1 to mas_x[0] do
              MainGraphForm.stringgrid4.Cells[o+TrigCol*2+2-1,cur_mas]:=' --'
          else
            for o:=TrigCol*2+2 to TrigCol*2+2+mas_x[0]-1 do begin
              pr1:=0;
              for p:=1 to strtoint(avtomat_mili[i].perehodi[j,-1]) do
                if avtomat_mili[i].perehodi[j,p]=MainGraphForm.StringGrid4.Cells[o,0] then
                  pr1:=1 else
                if avtomat_mili[i].perehodi[j,p]='-'+MainGraphForm.StringGrid4.Cells[o,0] then
                  pr1:=-1;
              if pr1=1 then
                MainGraphForm.StringGrid4.Cells[o,cur_mas]:='1';
              if pr1=-1 then
                MainGraphForm.StringGrid4.Cells[o,cur_mas]:='0';
              if pr1=0 then
                MainGraphForm.StringGrid4.Cells[o,cur_mas]:='--';
            end;
          for o:=TrigCol*2+2+mas_x[0] to TrigCol*2+1+mas_x[0]+mas_y[0] do begin
            MainGraphForm.stringgrid4.Cells[o,cur_mas]:='0';
            for p:=1 to avtomat_mili[i].y[j,0] do
              if 'y'+inttostr(avtomat_mili[i].y[j,p])=MainGraphForm.stringgrid4.Cells[o,0] then
                MainGraphForm.stringgrid4.Cells[o,cur_mas]:='1';
          end;
          if MainGraphForm.RadioGroup4.ItemIndex=0 then
            //D-triggers
            for o:=1 to TrigCol do
              MainGraphForm.stringgrid4.cells[o+1+TrigCol*2+mas_x[0]+mas_y[0],cur_mas]:=MainGraphForm.stringGrid4.cells[o+TrigCol+1,cur_mas]
          else
            //RS-triggers
            for o:=1 to TrigCol do begin
              if (MainGraphForm.stringgrid4.cells[o,cur_mas]='0') and (MainGraphForm.stringgrid4.cells[o+1+TrigCol,cur_mas]='0') then begin
                MainGraphForm.stringgrid4.cells[o*2+TrigCol*2+mas_x[0]+mas_y[0],cur_mas]:='--';
                MainGraphForm.stringgrid4.cells[o*2+1+TrigCol*2+mas_x[0]+mas_y[0],cur_mas]:='0';
              end else
              if (MainGraphForm.stringgrid4.cells[o,cur_mas]='0') and (MainGraphForm.stringgrid4.cells[o+1+TrigCol,cur_mas]='1') then begin
                MainGraphForm.stringgrid4.cells[o*2+TrigCol*2+mas_x[0]+mas_y[0],cur_mas]:='0';
                MainGraphForm.stringgrid4.cells[o*2+1+TrigCol*2+mas_x[0]+mas_y[0],cur_mas]:='1';
              end else
              if (MainGraphForm.stringgrid4.cells[o,cur_mas]='1') and (MainGraphForm.stringgrid4.cells[o+1+TrigCol,cur_mas]='0') then begin
                MainGraphForm.stringgrid4.cells[o*2+TrigCol*2+mas_x[0]+mas_y[0],cur_mas]:='1';
                MainGraphForm.stringgrid4.cells[o*2+1+TrigCol*2+mas_x[0]+mas_y[0],cur_mas]:='0';
              end else
              if (MainGraphForm.stringgrid4.cells[o,cur_mas]='1') and (MainGraphForm.stringgrid4.cells[o+1+TrigCol,cur_mas]='1') then begin
                MainGraphForm.stringgrid4.cells[o*2+TrigCol*2+mas_x[0]+mas_y[0],cur_mas]:='0';
                MainGraphForm.stringgrid4.cells[o*2+1+TrigCol*2+mas_x[0]+mas_y[0],cur_mas]:='--';
              end;
            end;
          inc(cur_mas);
      end;
  ...
  //структурная таблица автомата в СГ
    ...
    for i:=1 to SostNum do
      for j:=1 to strtoint(avtomat_mura[i].perehodi[0,0]) do begin
          ...
          MainGraphForm.stringGrid4.cells[TrigCol+1,cur_mas]:=' a'+avtomat_mura[i].perehodi[j,0];
          for g:=2 to TrigCol+1 do begin
          ...
          if avtomat_mura[i].perehodi[j,-1]='0' then
            ...
            for o:=TrigCol*2+2 to TrigCol*2+2+mas_x[0]-1 do begin
              ...
              for p:=1 to strtoint(avtomat_mura[i].perehodi[j,-1]) do
                if avtomat_mura[i].perehodi[j,p]=MainGraphForm.StringGrid4.Cells[o,0] then
                  pr1:=1 else
                if avtomat_mura[i].perehodi[j,p]='-'+MainGraphForm.StringGrid4.Cells[o,0] then
                  pr1:=-1;
              if pr1=1 then
          ...
          for o:=TrigCol*2+2+mas_x[0] to TrigCol*2+1+mas_x[0]+mas_y[0] do begin
            ...
            for p:=1 to avtomat_mura[{strtoint(avtomat_mura[}i{].perehodi[j,0])}].y[0] do
              if 'y'+inttostr(avtomat_mura[{strtoint(avtomat_mura[}i{].perehodi[j,0])}].y[p])=MainGraphForm.stringgrid4.Cells[o,0] then
                MainGraphForm.stringgrid4.Cells[o,cur_mas]:='1';
          end;
          if MainGraphForm.RadioGroup4.ItemIndex=0 then
            //D-triggers
...
begin
  MainGraphForm.memo2.Clear;
  for i:=TrigCol*2+mas_x[0]+2 to TrigCol*2+1+mas_x[0]+mas_y[0] do begin
    prov:=false;
    st:=MainGraphForm.stringgrid4.cells[i,0]+'=';
    for j:=1 to MainGraphForm.stringgrid4.RowCount-1 do
      if MainGraphForm.stringgrid4.cells[i,j]='1' then begin
        prov:=true;
        if st[length(st)]<>'=' then begin
          st:=st+' V ';
        end;
        if MainGraphForm.RadioGroup3.itemindex=0 then
          for g:=TrigCol*2+2 to TrigCol*2+1+mas_x[0] do
            if MainGraphForm.stringgrid4.cells[g,j]='1' then
              st:=st+MainGraphForm.stringgrid4.cells[g,0] else
              st:=st+'not('+MainGraphForm.stringgrid4.cells[g,0]+')';
      ...
      if not prov then
        st:=st+'0';
      cut_dup(st);
      MainGraphForm.memo2.Lines.Add(st);

  end;
  for i:=TrigCol*2+2+mas_y[0]+mas_x[0] to MainGraphForm.stringgrid4.colcount-1 do begin
    prov:=false;
    st:=MainGraphForm.stringgrid4.cells[i,0]+'=';
    for j:=1 to MainGraphForm.stringgrid4.RowCount-1 do
      if MainGraphForm.stringgrid4.cells[i,j]='1' then begin
...
procedure Draw_shema;
var pr,k,i,j,g,x1,y1,y2,m:integer;
    mas:array [1..100] of string;
    st,st1:string;
...
begin
  MainGraphForm.image2.canvas.Rectangle(xx,yy,xx+40,yy+60);
  MainGraphForm.image2.canvas.moveto(xx+10,yy);
  MainGraphForm.image2.canvas.lineto(xx+10,yy+60);
  MainGraphForm.image2.canvas.moveto(xx+30,yy);
  MainGraphForm.image2.canvas.lineto(xx+30,yy+60);

  MainGraphForm.image2.canvas.moveto(xx-10,yy+15);
  MainGraphForm.image2.canvas.lineto(xx,yy+15);
  MainGraphForm.image2.canvas.moveto(xx+40,yy+15);
  MainGraphForm.image2.canvas.lineto(xx+50,yy+15);
  if trigtype='D' then begin
    MainGraphForm.image2.canvas.textout(xx+15,yy+5,'D');
    MainGraphForm.image2.canvas.textout(xx-20,yy+5,'D'+inttostr(i));
    MainGraphForm.image2.canvas.textout(xx+50,yy+5,'Q'+inttostr(i));
    MainGraphForm.image2.canvas.textout(xx+1,yy+5,'D');
  end else begin
    MainGraphForm.image2.canvas.moveto(xx-10,yy+45);
    MainGraphForm.image2.canvas.lineto(xx,yy+45);
    MainGraphForm.image2.canvas.textout(xx+15,yy+5,'RS');
    MainGraphForm.image2.canvas.textout(xx-20,yy+5,'R'+inttostr(i));
    MainGraphForm.image2.canvas.textout(xx-20,yy+35,'S'+inttostr(i));
    MainGraphForm.image2.canvas.textout(xx+50,yy+5,'Q'+inttostr(i));
    MainGraphForm.image2.canvas.textout(xx+1,yy+5,'R');
    MainGraphForm.image2.canvas.textout(xx+1,yy+35,'S');
...
var i:integer;
begin
  MainGraphForm.image2.canvas.rectangle(xx,yy,xx+30,yy+15*(size+1));
  for i:=1 to size do begin
    MainGraphForm.image2.canvas.moveto(xx,yy+i*15);
    MainGraphForm.image2.canvas.lineto(xx-15,yy+i*15);
    if pos(' ',mas[num])<>0 then begin
    MainGraphForm.image2.canvas.textout(xx-40,yy+i*15-7,copy(mas[num],1,pos(' ',mas[num])-1));
    delete(mas[num],1,pos(' ',mas[num]));
    end else begin
    MainGraphForm.image2.canvas.textout(xx-40,yy+i*15-7,copy(mas[num],1,length(mas[num])));
    delete(mas[num],1,length(mas[num]));
    end;
  end;
  MainGraphForm.image2.canvas.textout(xx+10,yy+5,'&');
  if m>1 then begin
    MainGraphForm.image2.canvas.moveto(xx+30,yy+(size+1)*7);
    MainGraphForm.image2.canvas.lineto(xx+30+5*num,yy+(size+1)*7);
    MainGraphForm.image2.canvas.lineto(xx+30+5*num,y1+num*10);
    MainGraphForm.image2.canvas.lineto(350,y1+num*10);
  end else begin
    MainGraphForm.image2.canvas.moveto(xx+30,yy+(size+1)*7);
    MainGraphForm.image2.canvas.lineto(xx+40,yy+(size+1)*7);
    MainGraphForm.image2.canvas.textout(xx+40,yy+(size)*7,out_name);
  end;
end;

procedure Draw_or(xx,yy,size:integer;out_name:string);
begin
  MainGraphForm.image2.canvas.rectangle(xx,yy,xx+30,yy+10*(size+1));
  MainGraphForm.image2.canvas.textout(xx+10,yy+5,'1');
  MainGraphForm.image2.canvas.moveto(xx+30,yy+(size+1)*5);
  MainGraphForm.image2.canvas.lineto(xx+40,yy+(size+1)*5);
  MainGraphForm.image2.canvas.textout(xx+40,yy+size*5,out_name);
end;

begin
  MainGraphForm.Image2.canvas.rectangle(0,0,753,25000);
  for i:=1 to TrigCol do
    if MainGraphForm.radiogroup4.ItemIndex=0 then
  ...
  y1:=30;
  y2:=y1;
  for i:=0 to MainGraphForm.memo2.Lines.Count-1 do begin
    if y2<>30 then
      y1:=y2+30;
    y2:=y1;
    st1:=copy(MainGraphForm.memo2.Lines[i],1,2);
    if (st1[1]='y') or (st1=' D') or (st1=' R') or (st1=' S')  then begin
     st:=MainGraphForm.memo2.Lines[i];
    if (i<MainGraphForm.memo2.Lines.Count-1) then begin
      st1:=copy(MainGraphForm.memo2.Lines[i+1],1,2);
    if (st1[1]='y') or (st1=' D') or (st1=' R') or (st1=' S') then
    begin end
    else begin
      st:=st+MainGraphForm.memo2.Lines[i+1];
      if (i<MainGraphForm.memo2.Lines.Count-2) then begin
      st1:=copy(MainGraphForm.memo2.Lines[i+2],1,2);
      if (st1[1]='y') or (st1=' D') or (st1=' R') or (st1=' S') then begin
      end else begin
        st:=st+MainGraphForm.memo2.Lines[i+2];
        if (i<MainGraphForm.memo2.Lines.Count-3) then begin
          st1:=copy(MainGraphForm.memo2.Lines[i+3],1,2);
          if (st1[1]='y') or (st1=' D') or (st1=' R') or (st1=' S') then begin
          end else
            st:=st+MainGraphForm.memo2.Lines[i+3];
            if (i<MainGraphForm.memo2.Lines.Count-4) then begin
              st1:=copy(MainGraphForm.memo2.Lines[i+4],1,2);
              if (st1[1]='y') or (st1=' D') or (st1=' R') or (st1=' S') then begin
              end else
                st:=st+MainGraphForm.memo2.Lines[i+4];
            end;
          end;
        end;
      end;
