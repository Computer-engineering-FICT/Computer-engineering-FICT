...
uses
  ...
  Dialogs, StdCtrls, ExtCtrls,unit2,Unit5;
...
begin
  ...
  begin
    ...
    begin
      Form5.ListBox1.Items.Add(' ');
      for j:=0 to length(CompletePathArr[i])-1 do
        Form5.ListBox1.Items[i]:=Form5.ListBox1.Items[i]+ inttostr(CompletePathArr[i,j])+'  ' ;
    end;
    for i:=1 to CycleCount do
    begin
      Form5.ListBox2.Items.Add('');
      for j:=0 to length(CycleArr[i])-1 do
        Form5.ListBox2.Items[i-1]:=Form5.ListBox2.Items[i-1]+ inttostr(CycleArr[i,j])+'  ' ;
    end;
...
if AllRight then
 //begin
  MessageDlg('There are no mistakes in this graph ',mtInformation,[mbok],0) ;
...
begin
    ...
    if VerList[i].exist then
      ...
      if VerList[i].Vid=l then
        ...
        begin
          empty:=false;
          Form3.ComboBox1.Items.Add(IntToStr(VerList[i].Num)+'/связь по False');
        end;
    end;
  if empty then
   MessageDlg('We can not add the point to the graph, bebause all connections are not avaliable!',mtWarning,[mbok],0)
  else
  begin
...
begin
  ...
  begin
    ...
    if VerList[i].Vid=l then
      ...
      begin
        empty:=false;
        Form4.ComboBox1.Items.Add(IntToStr(VerList[i].Num)+'/ Связь по False');
      end;
  end;
  if empty then
    MessageDlg('We can not add the connection to the graph, because all points are not free!',mtWarning,[mbok],0)
  else
  begin
    ...
    Form4.Caption:='Добавление  связи';
    Form4.ShowModal;
    Form4.Free;
  end;
end;{Ins ConnectionДобавление связи }
...
begin
  ...
  Form4.Caption:='Удаление вершины';
  Form4.ShowModal;
  Form4.Free;
end; {DelУдаление вершины}
...
begin
  ...
  if empty then
    MessageDlg('Graph has not connection at all.Connect please the points!',mtWarning,[mbok],0)
  else
  begin
    for i:=1 to MaxVerNum do
    if VerList[i].exist then
...
Form5:=TForm5.Create(Self);
  ...
  for i:=0 to VerCount-1 do
    ...
    case Vid of
      k:PutEllipse(Col,row,Num);
      o:PutRect(Col,Row,Num,Value);
      l:PutRomb(Col,Row,Num,Value)
    end;
  if NextMain>-1 then
