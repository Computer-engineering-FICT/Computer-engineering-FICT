...
begin
  Variant := 0;
  if NumFromStr(cbPrev.Items[cbPrev.ItemIndex],'/')=StrToInt(cbPrev.Items[cbPrev.ItemIndex]) then
  ShowError('Такое соединение невозможно')
  else
  begin
    Vp:=NumFromStr(cbPrev.Items[cbPrev.ItemIndex],'/');
    Vn:=NumFromStr(cbNext.Items[cbNext.ItemIndex],'/');
    TableSv[findver(Vp),Findver(Vn)]:=true;
    case VerList[Findver(Vp)].Vid of
       o:Variant:=0;
       l:if IsDelim('/',cbPrev.Items[cbPrev.ItemIndex]) then Variant:=1 else Variant:=0;
       b:Variant:=2;
     end;
    ...
    begin
      if (Vid=o) or (Vid=b) then
        NextMain:=Vn;
      if Vid=l then
        if IsDelim('/',cbPrev.Items[cbPrev.ItemIndex])  then
          NextAlt:=Vn
        else
          NextMain:=Vn;
    end;
...
Vp:=NumFromStr(cbPrev.Items[cbPrev.ItemIndex],'/');
   ...
   if (VerList[Vp].NextMain=Vn)and(not IsDelim('/',cbPrev.Items[cbPrev.ItemIndex]))or(Verlist[Vp].NextAlt=Vn)and(IsDelim('/',cbPrev.Items[cbPrev.ItemIndex])) then
   with  VerList[FindVer(Vn)] do
   begin
       case VerList[FindVer(Vp)].Vid of
         o:Variant:=0;
       ...
       DisConnect(VerList[FindVer(Vp)].Col,VerList[Findver(Vp)].Row,Col,Row,Variant);
        ...
        for i:=0 to Length(Prev)-1 do
          if Prev[i]=Vp then
           DelItem(prev,i);
        if IsDelim('/',cbPrev.Items[cbPrev.ItemIndex])  then          VerList[Vp].NextAlt:=-1
        else
