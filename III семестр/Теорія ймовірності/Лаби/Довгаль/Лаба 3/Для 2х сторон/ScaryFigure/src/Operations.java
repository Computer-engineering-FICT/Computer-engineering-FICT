public class Operations {
    ...
    static double calcTrap(double r, MyFunction func) throws Exception {

        double currentStep=0, square=0, step=Main.delta;
        for (;square<r;currentStep+=step){
            square+= (func.doOperation(currentStep)+func.doOperation(currentStep+step))/2*step;
            if (currentStep>=10/step) throw new Exception();
        }
        return currentStep;
    }
