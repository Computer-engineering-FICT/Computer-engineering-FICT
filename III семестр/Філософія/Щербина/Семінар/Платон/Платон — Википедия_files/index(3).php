...
{
   try {
      return wgLangNames[ wgUserLanguage ][ a.className.replace(/^.*?interwiki-/, '').replace(/\s.*$/, '') ][ 1 ] - wgLangNames[ wgUserLanguage ][ b.className.replace(/^.*?interwiki-/, '').replace(/\s.*$/, '') ][ 1 ];
    }
    catch (e) {
...
addOnloadHook( function () {
    ...
    try {
        ...
        for (i=0;i<childs.length;i++) {
            ...
            if ( typeof wgLangIWlinks[ childs[i].className.replace(/^.*?interwiki-/, '').replace(/\s.*$/, '') ] != 'undefined' && wgLangIWlinks[ childs[i].className.replace(/^.*?interwiki-/, '').replace(/\s.*$/, '') ]) {
                keeps.push(childs[i]);
            }
        }
