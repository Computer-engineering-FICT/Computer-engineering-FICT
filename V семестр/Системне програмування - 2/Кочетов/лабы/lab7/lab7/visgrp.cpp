...
{
	if(rt->ndOp>_inr||rt->ndOp<_if||(rt->ndOp>=_to&&rt->ndOp<=_step))
	{
		...
		while((c=cpr[rt->ndOp&(~_lVlu)][n])!=0)// цикл прогляду шаблона
		{
			...
			switch(c)	// аналіз літери шаблона
			{
				case 7: 
					...
					break;
				case 6: 
					opCnt--;// повернення до першого аргумента
				case -1:
					break;
				case -2:
					mdCnt=0; break;
				case 1:
					/*case 'f':*/ if(mode!=0&&rt->ndOp>=_if&&rt->ndOp<begOprtr-8)
						printf(" ");
					printf("%s",oprtr[rt->ndOp&(~_lVlu)]); 
					if(rt->ndOp>=begOprtr-8)
						mode=0;
					else
						mode=1;
				break;
				...
				case 4:
					if(opCnt)
						rt0=rt->pstNd;	// вибір аргумента
					else
						rt0=rt->prvNd;	// перевірка потреби обрамлення
					if(rt0)
					{
						if(c=='y')
							bC=fpr[rt->ndOp]>fpr[rt0->ndOp]&&rt0&&rt0->ndOp!=_opbz;
						else if(c==5)
						{
							if(rt0&&rt0->ndOp!=_brkz)
								bC=1;
						}
					else
						if(c==4)
							...
							bC=fpr[rt->ndOp&(~_lVlu)]>=fpr[rt0->ndOp&(~_lVlu)]&&rt0;
						if(bC)
							prOpBr(rt0);	// обрамлення дужками
						prLxTxt(rt0);		// рекурсивний виклик відтворення
						if(bC)
							prClBr(rt0);	// обрамлення дужками
						}
				case 'z':
					opCnt++;		// підготовка наступного аргумента
					break;
				default:
					printf("%c",c);
	}
