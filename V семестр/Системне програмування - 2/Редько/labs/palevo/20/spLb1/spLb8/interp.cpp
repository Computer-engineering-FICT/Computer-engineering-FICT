...
//			vp1=SmIntrp(nd->prvNd,1);
			...
			}else{if(nd->pstNd->dataType<_f) acc32._id=vp1->_id=vp2->_id;
			else acc32._id=vp1->_id=vp2->_fd;}}
		prAss(nd);
		return &acc32;}
	 if(nd->prvNd&&nd->ndOp!=_ixbz)
	 {vp1=SmIntrp(nd->prvNd,1);
...
//вирівнювання типів
	...
	if(dfnFlg!=0||(nd->ndOp>=_eosP&&nd->ndOp<=_EOS))
		return NULL;
	if(//nd->dataType>=_f	||
		(nd->prvNd!=0&&nd->prvNd->dataType>=_f)
		||(nd->pstNd!=0&&nd->pstNd->dataType>=_f))
	{if(nd->prvNd!=0&&nd->prvNd->dataType<_f&&nd->ndOp!=_cln)vp1->_fd=vp1->_id;
	 if(nd->pstNd!=0&&nd->pstNd->dataType<_f&&nd->ndOp!=_ixbz)vp2->_fd=vp2->_id;
// сюди вставляти потрібні операції
 ...
 break;
		 ...
		 case _ixbz:
			 if(nd->prvNd)
				acc32._fd=((float*)vp1)[vp2->_id];break;
		 case _brkz:
			 if(nd->prvNd)
				acc32._fd=vp2->_fd; 
	 }}else{
        ...
        mov        dword ptr [acc32+4],edx
		 ...
		 case _ixbz:
			 if(nd->prvNd)
				acc32._id=((unsigned*)vp1)[vp2->_id]; break;
		 case _dcr:
			 if(nd->prvNd==0)
				acc32._id=--(vp2->_id);
			 else acc32._id=(vp1->_id)--; break;
		 case _inr:
			 if(nd->prvNd==0)
				acc32._id=++(vp2->_id);
			 else acc32._id=(vp1->_id)++; break;
