...
#include <stdlib.h>
unsigned nbBlk=0;
extern struct recrdSMA	ftTbl[179];// таблиц€ припустимост≥ тип≥в дл€ операц≥й
...
//		 nd->pstNd->resLength=lnCod[nd->pstNd->dataType];
	...
	}else if(nd->ndOp==_mul)//унарна *
	{if(nd->prnNd)SmAnDcl(tpCod+cdPtr,nd->pstNd);
	}else if(nd->ndOp==_ixbz)
	{SmAnDcl((tpCod+cdPtr)|cdCns,nd->prvNd);
	 if(nd->pstNd->ndOp==_srcn)
...
enum datType SmAnlzr(struct lxNode * nd,	// вказ≥вник на початок масиву вузл≥в 
 		  int nR)	// номер кореневого вузла
{enum datType tPrv, tPst; int lnPrv, lnPst;
...
//  nd->resLength=lnCod[tPrv];
	...
	tPrv=(enum datType)nd->dataType;}
	 ...
	 {ftImp.oprd1=tPrv; if(tPrv!=_v)ftImp.ln1=lnPrv;else ftImp.ln1=lnPrv=0;
		 ftImp.oprd2=tPst; if(tPst!=_v)ftImp.ln2=lnPst;else ftImp.ln2=lnPst=0;
		 ftImp.oprtn=nd->ndOp;
		 struct recrdSMA*
