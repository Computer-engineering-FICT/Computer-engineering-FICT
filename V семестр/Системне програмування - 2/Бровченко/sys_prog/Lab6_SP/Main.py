...
from Lab6_SP.Stack import Stack
from Lab6_SP.Utils import is_var, brackets_checker, is_num, tokens
    ...
    if not check_syntax(inp):
        try:
            ...
            for line in inp_arr:
                ...
                elif line_tokens[0] not in types and line.find(":=") != -1 and line_tokens[0] in var_dict.keys():
                    var_dict[line_tokens[0]].value = count_value(line[line.index(":=") + 3:], var_dict[line_tokens[0]].var_type)
                elif line_tokens[0].find("["):
