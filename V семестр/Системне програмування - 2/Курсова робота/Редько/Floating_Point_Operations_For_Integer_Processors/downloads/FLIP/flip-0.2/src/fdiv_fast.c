...
/*---------------------------------------------------------------------------
  This implementation of division is much faster than the one in "fdiv.c".
  ...
  processors are explained later. Apart from the division of the mantissas
  all other computations are performed in the same way as in "fdiv.c".
...
{
  float32 _x, _y;
  bits32 _xFrac, _yFrac,_xMant, _yMant;
...
#ifdef __ST220__  // code specific to ST220

  bits32 _q1;
  sbits32 _w2, _q2, _q3;

#else // code specific to ST231
...
---------------------------------------------------------*/
  ...
  _rBExp = _xBExp - _yBExp + 0x7F;          // Compute the exponent of the result.
...
/*----------------------------------------------------------------------------------
  Brief description of the division implementation for ST220.
  The division "_xMant / _yMant" where 1 <= _xMant, _yMant < 2, is performed in
  3 following steps for ST220.
  Step 1: Obtain an approximation of "1 / _yMant" through a degree 3 polynomial.
  Step 2: Obtain the first few bits of the quotient by multiplying _xMant by the
          above approximation. Refine this quotient by 2 high-radix SRT iterations.
  Step 3: Obtain the remainder in order to have the last bit correct and prepare
          the quotient for rounding. This step is common for both processors.
...
/*-----------------------------------------------------------------------------------
  ...
  Initially _xMant and _yMant are stored as 01.X...X00...0 (2,23). This is a machine
...
/*----------------------------------------------------------------------------------
  The multiplication is performed using an intrinsic function "_flip_32h_smul_32_16"
  which returns top 32 bits of the 48-bit product obtained from a signed 
  32 * 16 multiplication.
----------------------------------------------------------------------------------*/
...
/*------------------------------------------------------------------------------
  ...
  1 - 2^9 < z = Y*M < 1 + 2^9.
  Prescale the dividend and the divisor (multiply by M) so that z approaches
  1 and w0 approaches the quotient (denote q). M is truncated to 16 bits.
------------------------------------------------------------------------------*/

  _z = _flip_32h_smul_32_16( _yMant, _poly_approx) << 1;  // 0X.X...X0 (2,29).
  _w0 = _flip_32h_smul_32_16( _xMant, _poly_approx);      // 00X.X...X (3,29).
...
/*-----------------------------------------------------------------------------
  ...
  position to avoid any overflow. So assuming the form of w0 as 0.0X...X00 the
  first 9 bits of the quotient are obtained as ROUND(2^9*w0 + 2^-1) which
  gives the integral bits. No need to shift w0 explicitly as the position of 
  the point is taken care of while rounding.
-----------------------------------------------------------------------------*/

  _q1 = ( _w0 + 0x00200000 ) & 0xFFC00000;  // XXXXXXXXXX.00...0 (10,0).

/*-----------------------------------------------------------------------------
  SRT iteration      : w[j+1] = 2^9 * w[j] - q[j+1] * z.
  Quotient selection : q[j+1] = ROUND(2^9 * w[j] + 2^-1).
  Since q[j+1]*z is of the form 0XXXXXXXXXXX.X...X (12,20), 2^9*w[j] must be
...
/*-------------------------------------------------------------------
  The final quotient is formed as shown below. It can be of 2 forms,
...
/*----------------------------------------------------------------------------------
  ...
  Step 2: Prescale the divisor and the dividend by multiplying with the above
          approximation. Obtain the quotient by performing 1 iteration of 
...
/*-----------------------------------------------------------------------------------
  All the variables and intermediate values are encoded on a 32 bits register. 
  Initially _xMant and _yMant are stored as 01.X...X00...0 (2,23) and 1.XX...X (1,23)
  respectively. This is a machine interpretation that shows the position of the 
  decimal point of each value. In (I,F) I is the no. of integral bits and F, the no.
  of significant fractional bits. The point is placed at the best possible position
  in order to have guard bits (prevent loss of precision) on the right and to avoid
...
/*------------------------------------------------------------------------------
  ...
  1 - 2^16 < z = Y*M < 1 + 2^16.
  Prescale the dividend and the divisor (multiply by M) so that z approaches
  1 and w0 approaches the quotient (denote q).
------------------------------------------------------------------------------*/

  _z = _flip_32h_umul_32_32( _yMant, _poly_approx);  // 0X.XX...X (2,30).
    
  _w0 = _flip_32h_umul_32_32( _xMant, _poly_approx) << 2;  // X.XX...X00 (1,29).

/*-----------------------------------------------------------------------------------
  Brief description of the Goldschmidt algorithm.
  To find quotient q = _xMant / _yMant (X/Y), Goldschmidt algorithm tries to find a
  sequence K1, K2, K3... such that Y*K1*K2*...*Ki approaches 1 as i goes infinity.
  Hence, qi = X*K1*K2*...Ki approaches q. Here, computing K1 and K2 allows to have a
  quotient of required precision (25 bits). After finding K1, if Y*K1 = 1 - eps,
  select K2 = 1 + eps, which gives Y*K1*K2 = 1 - eps^2. So, X*K1*K2 is now more
  closer to q than X*K1.
------------------------------------------------------------------------------------*/

/*------------------------------------------------------------------------------------
  The above approximation gives K1 = M which is multiplied by the dividend (X*K1) and
  the divisor (Y*K1) above in the prescaling step. Perform the first iteration of
  Goldschmidt and compute K2 = 1 + 1 - Y*K1 (2's complement). Obtain the new divisor
  (approaches 1) and the new dividend (approaches q) with greater precision.
-------------------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------------
  The final quotient is obtained as ROUND(2^25*_w1 + 2^-1). There can be 2 forms of
  _w0, 00.1X...X0 or 01.XX...X0, depending on the dividend and the divisor.
----------------------------------------------------------------------------------*/
...
/*---------------------------------------------------------------------------------
  Either of the implementation gives a 27-bit quotient (first bit is always 0) and
  since a 25-bit normalized quotient is required, it is shifted right by 1 bit in
  the case when "_xMant >= _yMant".
---------------------------------------------------------------------------------*/

  if( _xMant >= _yMant )  // q: 00000 01.XX...X (7,25).
    {
      _shift = 7;
      _q = _q >> 1;
    }
  else                    // q: 00000 00.1X...X (7,25).
    {
      _shift = 6;
      _rBExp = _rBExp - 1;
    }
...
/*--------------------------------------------------------------------------------
  Step3:
  Remainder is obtained as _xMant - _yMant * _q in order to have the last bit
  correct for final rounding (round-to-nearest-even) of the quotient. Since _yMant
  and _q are of 24 and 25 bits, a full width multiplication (32*32 => 32 bits) is
  performed. This step is executed nearly 4 times faster on ST231 than on ST220.
--------------------------------------------------------------------------------*/

  _prod_quo_div = _flip_32h_umul_32_32( ( _yMant << 1 ), ( _q << _shift ) );
  _rem = _xMant - _prod_quo_div;
