...
public class Graf {
	...
	public void config() {
		Device dev1 = new Device(1,// ідентифікатор
				0,// черга
				1,// ресурс
				0,// вільні ядра
				"Процесор",// назва
		...
		Device dev2 = new Device(2, 0, 1, 0, "Північний міст", 20.0);
		Device dev3 = new Device(3, 0, 0, 1, "Південний міст", 30.0);

		dev1.linkOutDevice(dev1, dev2);
		dev1.linkOutProbability(1, 2);

		dev2.linkOutDevice(dev1, dev3);
		dev2.linkOutProbability(3, 4);

		dev3.linkOutDevice(dev2, dev1);
		dev3.linkOutProbability(5, 6);

		State head = new State(dev1, dev2, dev3);
	...
	public void buildTree() {
		...
		for (Transition d : uniqueArcs) {
			System.out.println(d);
		}
		
	}

	public int findEndLEvel() {
		Transition[] transitions = new Transition[uniqueArcs.size()];
		transitions = uniqueArcs.toArray(transitions);
		int index = 0;
		for (int i = transitions.length - 2; i > 0; i--) {
			if (transitions[i].headNode == 0 && transitions[i].id == 0) {
				index = i;
				break;
			}
		}
	...
	public void calcLoadingDevice() {
		...
		System.out.println("Одне Ядро          "
				+ String.format("%3.15f", loading[1]));
		System.out.println("Два Ядра           "
				+ String.format("%3.15f", loading[0]));
		System.out.println("Північний міст     "
				+ String.format("%3.15f", loading[2]));
		System.out.println("Південний міст     "
				+ String.format("%3.15f", loading[3]));
		System.out.println("Оперативна пам*ять "
				+ String.format("%3.15f", loading[4]));
		System.out.println("Відео процесор     "
				+ String.format("%3.15f", loading[5]));
		...
		uniqueStates.get(indexMax).printState(this);
		System.out.println();
