...
namespace PSCLaba04Alex
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Main thread ends here, press any key to exit");
            Console.ReadKey();
        }
		...
		static void f2()
		{
			...
			Matrix mB = new Matrix(n); mB.fill();
		...
		static void f3()
		{
			...
			Vector vB = new Vector(n); vB.fill();
			Matrix mD = new Matrix(n); mD.fill();
			Matrix mA = new Matrix(n); mA.fill();
			Matrix mB = new Matrix(n); mB.fill();
    ...
    public class Vector
    {
		...
		public int[] v;
		public Vector(int n = defaultSize)
		{
			v = new int[n];
		}
		public Vector(int[] v)
    ...
    public class Matrix
    {
		...
		public Matrix(int n = Vector.defaultSize)
		{
		    m = new Vector[n];
		    for (int i = 0; i < n; i++)
		    {
	...
	public class VectorsAndMatricesArithmetic
	{
		...
		public static int getMax(Matrix m)
		{
		    int max = getMax(m.m[0]);
		    for (int i = 1; i < m.m.Length; i++)
		    {
		        int m = getMax(m.m[i]);
		        if (max < m)
		        {
