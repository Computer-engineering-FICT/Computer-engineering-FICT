...
class window_1(Frame):
    ...
    def table(self,nodes_num):
        ...
        self.entry_labels_first_row = [Label(self, text = "n" + str(i)) for i in range(nodes_num)]
        self.entry_labels_first_column = [Label(self, text = "n" + str(i)) for i in range(nodes_num)]
        ...
        self.graph_show = Button(self, width = 20, text = "Згенерувати граф", command = self.draw)
        self.task_calculate = Button(self, width = 20, wraplength = 180,text = "Розфарбувати граф", command = self.color_graph)
