    ...
    def callback():
        ...
        try:
            ...
            if entr < 256:
                ...
                msg = messagebox.showinfo('Error', 'Power must be less than 256 \n Power A = 255')
        except:
            msg = messagebox.showinfo('Error', 'Please input only numeric')
...
def create_universal_set(event):
    ...
    def callback_to_range(event):
        try:
            ...
            c = list(C)
            try:
                if over > a[len(sorted(a))-1] and over > b[len(sorted(b))-1] and over > c[len(sorted(c))-1]:
                    Add = set(range(begin,over))
                    U.update(Add)
                    msg = messagebox.showinfo('Universal set', 'Universal set was created')
                else:
                    msg = messagebox.showinfo('Error', 'Universal set must be more than elements in sets')
            except:
                msg = messagebox.showinfo('Error', 'Sets are empty')
        except:
            msg = messagebox.showinfo('Error', 'Please input only numeric')
