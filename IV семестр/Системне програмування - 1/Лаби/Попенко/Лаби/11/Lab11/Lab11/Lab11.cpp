...
void firstVecSSE(HWND hWnd) {
	...
	{
		vectorSSE(dest, a, b, huge*4); //код, для якого потрібно виміряти час
	}
	GetLocalTime(&st);
...
void firstVecFPU(HWND hWnd) {
	...
	for (long i = 0; i<10000000; i++)
	{
		vectorFPU(dest, a, b, huge*4); //код, для якого потрібно виміряти час
	}
	GetLocalTime(&st);
	ten = 60000 * (long)st.wMinute
		+ 1000 * (long)st.wSecond
...
void cppVec(HWND hWnd) {
	...
	for (long i = 0; i<10000000; i++)
	{		
		skal(a, b, dest);				//код, для якого потрібно виміряти час
	}
	GetLocalTime(&st);
	ten = 60000 * (long)st.wMinute
		+ 1000 * (long)st.wSecond
