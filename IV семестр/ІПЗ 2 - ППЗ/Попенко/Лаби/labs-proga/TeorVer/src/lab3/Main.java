...
public class Main {
	...
	public static void main(String[] args) {
		...
		int countPoint = 100;
		double[][] sX;
		double[][] sY = integrate(yF, 0, countPoint, 0, 1);

		for (int i = 0; i < n; i++) {
			double rr = random.nextDouble();
			double yi = rr * sY[0][sY[0].length - 1];
			y[i] = randomSign() * solution(sY, yi);
			sX = integrate(xF, y[i], countPoint, 0, 1);
			double xi = random.nextDouble() * sX[0][sX[0].length - 1];
			x[i] = randomSign() * solution(sX, xi);
