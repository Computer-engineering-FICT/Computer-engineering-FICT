link ewh:16
link l1:rdm
link l2:rdd
link l3:ct
link m:z,z,14,13,12,11,10,z,z,z,z,z
link ra:z,7,6,5
link rb:z,2,1,0
\----------Программа----------
dw 10h:2032h
dw 12h:1210h
dw 14h:2034h
dw 16h:4801h
dw 18h:2032h
dw 1ah:1218h
dw 1ch:2034h
dw 1eh:4802h
dw 20h:852ah
dw 22h:5040h
dw 24h:38ffh
dw 26h:2032h
dw 28h:1226h
dw 2ah:2034h
dw 2ch:0fffh
dw 2eh:30afh
dw 30h:38efh
dw 32h:20b2h
dw 34h:1232h
dw 36h:40efh
dw 38h:28b4h
dw 3ah:20b2h
dw 3ch:123ah
dw 3eh:40afh
dw 40h:28b4h
dw 42h:5800h

accept r7:10h

accept dev[1]:i,32h,34h,3,12
accept dev[2]:o,0B2h,0B4h,3,12
accept dev_buf[1]:0004h,0005h,8006h

\Макросы
macro mov приемник,источник:
{or приемник, источник,z;}
macro read_mem приемник:
{r;mov приемник,bus_d;cjp rdm,cp;}
\Данные
dw 03ffh:0ffh
dw 0ffh:0

{cjp nz,begin;}
\-------------выборка------------
begin {xor nil,r8,r8;ewh;oey;}
      {ewl;oey;or nil,r7,z;}
ll1   {r;cjp rdm,ll1;or r8,bus_d,z;}
\------------распаковка----------
{or rq,8000h,z;}
{and nil,r8,rq;cjp not zo,two_addr;} \ двухадресная команда

\одноадресная команда
{and r13,r8,111111111%;} \оставляем операнд или адрес в r13

\проверка адресации одноадресной команды
{and nil,r8,1000000000%;load rm,flags;}
{cjp rm_z,comma;} \прыжок если прямая адресация

{or nil,r13,r13;ewl;oey;}
ll2 {r;cjp rdm,ll2;or r13,bus_d,z;}
{ewl;oey;or nil,r13,z;}
ll3 {r;cjp rdm,ll3;or r12,bus_d,z;}

comma {or nil, r8,r8;jmap;oey;} \выполнение операции
\-------------------------------

\для двухадресной команды
two_addr {xor nil,r8,r8;oey;ewh;}
       {or r9,11100111%,z;}
       {ewa;ewb;oey;and nil,r8,r9;} \номера регистров в RA,RB
\-------------------------------
 {oey;or nil,r8,z;jmap;}

org 1100000%
{cjs nz,multiply;}
{cjp nz,next;}
\-------------------------------
multiply {load rm,z;} \подпрограмма умножения

{xor nil,r15,r2;load rn,flags;LDCT 16;}

{and r6,r15,8000h;}
{xor r6,r6,r2;}
{and r6,r6,8000h;}

{ADD R2,R2,Z;CJP not NO,prov;} \проверка знаков операндов
{and r2,r2,7fffh;}
prov {add r15,r15,z;cjp not no,label1;}
     {and r15,r15,7fffh;}

label1 {CJP NOT RM_C,MUL;}

       {add r5,r5,r14,z;load rm,flags;}
       {add r4,r4,r2,rm_c;}
MUL  {ADD srl,r2,r2,z;}
     {ADD sr.9,r14,r14,z;}
     {ADD SLL,R15,R15,Z;RPCT LABEL1;load rm,flags;cEM_C;}


     {add r4,r6,r4;load rm,flags;}
     {ADD R4,R4,Z;LOAD RN, FLAGS;}
     {CJP NOT RM_N,label5;}

     {xor r4,r4,0111111111111111%;} \перевод в ДК
     {xor r5,r5,1111111111111111%;}
     {add r5,z,nz;load rm,flags;}
     {add r4,z,rm_c;}

label5 {or r15,r4,z;}
       {or r14,r5,z;}
{crtn nz;}

     {cjp nz,next;}
org 100000%
     {cjs nz,Madd;}
     {cjp nz,next;}
\выполнение операции
{oey;jmap;or nil,r8,z;}
Madd \подпрограмма суммирования
     {add rb,rb,ra;}
    {crtn nz;}
    {cjp nz,next;}

\безусловный переход
org  11000000%
op_jmp{or rq,r13,z;}
{or r7,rq,z;cjp nz,begin;}

\условный переход
org  10000000%
op_cjmp{or rq,80h,z;}
{and nil,r15,rq;load rm,flags;}
{cjp not rm_z,next;}
{mov r7,r13;cjp nz,begin;}

\Ввод с ВУ
org 100000000%
op_in{or nil,r13,z;ewl;oey;}
input{i;cjp rdd,input;or r15,bus_d,z;}
{cjp nz,next;}

\Вывод с ВУ
org 101000000%
op_out{or nil,r13,z;oey;ewl;}
{or nil,r15,z;oey;o;cjp rdd,cp;}
{cjp nz,next;}
\------------------------
\savelow-сохранение Р14 в памяти
org 110000000%
{or r13,r13,z;oey;ewl;}
{w;or nil,r14,r14;oey;cjp rdm,cp;}
{cjp nz,next;}
\------------------------
\savehigh-сохранение Р15 в памяти
org 111000000%
{or r13,r13,z;oey;ewl;}
{w;or nil,r15,r15;oey;cjp rdm,cp;}
{cjp nz,next;}
\-----------------------
\load-загрузка в Р15 из памяти
org 1000000000%
{or r13,r13,z;ewl;oey;}
{read_mem r15;}
{cjp nz,next;}
\-----------------------
\movora-R<--R15
org 1001000000%
{or nil,r13,z;ewa;ewb;oey;}
{or rb,r15,r15;}
{cjp nz,next;}
\-----------------------
\movira-R15<---R
org 1010000000%
{or nil,r13,z;ewa;ewb;oey;}
{or r15,ra,z;}
{cjp nz,next;}
\----------------------
\Выход
org 1011000000%
exit{cjp nz,end;}
\----------------------
       next {add r7,r7,2,z;}
     {cjp nz,begin;}

end {}
